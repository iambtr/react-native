...
import { 
  AppRegistry,//注册app实例 才能使用rn
  /**
   * 会用到广播，接口，回调等方法来实现发送和接受通知以及通信
  ...
  // 图片组件
  Image,
  // 自带导航，RN 0.43版本开始，官方将停止维护Navigator，建议大家迁移到新的react-navigation库（文档地址需翻墙）
  // return了一个Navigator容器。initialRoute就是我们要放在这个导航容器中的根界面，也是第一个界面，
  // 这里我们放的是名为FirstView的界面，这个界面是由另一个js文件描述的，所以也要记得import。
  // 我们把它作为component参数的值，至于另一个name参数，真的也就是一个name而已，不太重要。
  // 然后紧跟着的configureScene是描述界面之间的过渡动画的，比如从右边滑出来啊或者从底部滑出来之类的，
  // 在node_modules/react-native/Libraries/CustomComponents/Navigator/NavigatorSceneConfigs.js文件中可以看到所有可以选的方式。
  // 接下来的内容直接写就可以了。
  Navigator,
  // 滚动组件
  ScrollView,
